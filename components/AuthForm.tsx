...
const router = useRouter();
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>)=> {
    ...
    try {
      ...
      // sign up with App write & create a plain link token
      if(type === 'sign-up'){
      ...
      <header className='flex flex-col gap-5 md:gap-8'>
        <Link href="/" className="cursor-pointer flex items-center gap-1">
          <Image
            ...
            alt="BH Bank logo"
          />
          <h1 className="text-26 font-ibm-plex-serif font-bold text-black-1">BHbank</h1>
        </Link>
        <div className='flex flex-col gap-1 md:gap-3'>
          <h1 className='text-24 lg:text-36 font-semibold text-gray-900'>
            {user ? 'Link Account' : type === 'sign-in' ? 'Sign In' : 'Sign Up'}

            <p className='text-16 font-gray-600 font-normal'>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              {type === 'sign-up' && (
                <>
                  ...
                  <div className='flex gap-4'>
                    <CustomInput control={form.control} name='state' placeholder='EX: Area Name' label='State' />

                    <CustomInput control={form.control} name='postalCode' placeholder='EX: 1200' label='Postal Code' />
                  </div>
              ...
              <div className='flex flex-col gap-4'>
                <Button type="submit" disabled={isLoading} className='form-btn'>
                  {isLoading ? (
                    <>
                      <Loader2 size={20} className='animate-spin' /> &nbsp; Loaading...
                    </>
